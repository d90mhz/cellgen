...
sub oai22	# parameters: lef filehandle, cell name, lambda, height, ratio, psize, nsize
{
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", $lcoords[0], $lcoords[1], $lcoords[2], $lcoords[3];
	print $lefhandle "    END\n  END gnd!\n  PIN out\n    DIRECTION OUTPUT ;\n    PORT\n    LAYER METAL2 ;\n";
	@lcoords = lefcoords ($lambda, 1.3+.72+.72*(2*$max_folds+$adjust), -.06, 1.585+.72+.72*(2*$max_folds+$adjust), .32);				# pin out
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", $lcoords[0], $lcoords[1], $lcoords[2], $lcoords[3];
	print $lefhandle "    END\n  END out\n  PIN in0\n    DIRECTION INPUT ;\n    PORT\n    LAYER METAL2 ;\n";
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", $lcoords[0], $lcoords[1], $lcoords[2], $lcoords[3];
	print $lefhandle "    END\n  END in1\n  PIN in2\n    DIRECTION INPUT ;\n    PORT\n    LAYER METAL2 ;\n";
	@lcoords = lefcoords ($lambda, .555, -.01, .935, .27);				# pin in2
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", $lcoords[0], $lcoords[1], $lcoords[2], $lcoords[3];
	print $lefhandle "    END\n  END in2\n  PIN in3\n    DIRECTION INPUT ;\n    PORT\n    LAYER METAL2 ;\n";
	@lcoords = lefcoords ($lambda, .555+.72+.72*($max_folds+$adjust), -.01, .935+.72+.72*($max_folds+$adjust), .27);				# pin in3
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", $lcoords[0], $lcoords[1], $lcoords[2], $lcoords[3];
	print $lefhandle "    END\n  END in3\n";
...
######## DIFF
	...
	if (iseven($pfolds))
	{	
		printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$max_folds, 1.15-(floor(100*$psize_each)/200), 1.65+.72*$max_folds, 1.15+(floor((100*$psize_each)+.5)/200));	# PDIFF for pmos middle if connected
	}
	else
	{
		printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$pfolds, 1.15-(floor(100*$psize_each)/200), 1.65+.72*$pfolds, 1.15+(floor((100*$psize_each)+.5)/200));		# PDIFF for pmos middle if not connected
	}
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72-.72*($max_folds+$pfolds+$adjust), 1.15-(floor(100*$psize_each)/200), 1.65-.72-.72*($max_folds+1+$adjust), 1.15+(floor((100*$psize_each)+.5)/200));	# PDIFF for pmos left

	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.36-.72*(2*$max_folds+$adjust+1), 2.325, 1.8+.72+.72*(2*$max_folds+$adjust), 2.88);	# NDIFF for vdd
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*($max_folds+$adjust+$nfolds+1), -.905-(floor(100*$nsize_each)/200), 1.65+.72*($max_folds+$adjust+$nfolds+1), -.905+(floor((100*$nsize_each)+.5)/200));	# NDIFF for nmos
...
######## METAL1
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.255+.72+.72*(2*$max_folds+$adjust), -.01, 1.635+.72+.72*(2*$max_folds+$adjust), .27);	# out pin pad
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.325+.72+.72*(2*$max_folds+$adjust), -.56, 1.555+.72+.72*(2*$max_folds+$adjust), 0);	# out to n-out
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.325, -.56, 1.555+.72+.72*(2*$max_folds+$adjust), -.33);	# out interconnect right
	if ($pfolds == 0)
	{
	       printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115+.72*($pfolds+3), -.56, .115+.72*($pfolds+3), .795);	# p-out to n-out connect wire
    }
    else
    {
	       printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115+.72*($pfolds+1), -.56, .115+.72*($pfolds+1), .795);	# p-out to n-out connect wire
    }
	## m1 for pmos side ######
	####### for in0 and in1
	$i = 0;
	...
	{
		$i++;
		if (($i < ($max_folds + 2)) && ($max_folds != 0))
		{
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115-(.72*($i-1)), 1.9, 1.555-(.72*($i-1)), 2.13);	# node left interconnect
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), 1.9, .115+(.72*($i+1)), 2.13);	# node right interconnect
		}
		$i++;
	}
	$i = 0;
	until ($i >= ($pfolds + 2))
	{
		...
		$i++;
		if (($i < ($pfolds + 2)) && ($max_folds != 0))
		{
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .55+(.72*(-$i)), 1.035, .89+(.72*(-$i)), 1.265);	# contact cover left
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .55+(.72*$i), 1.035, .89+(.72*$i), 1.265);		# contact cover right
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*(-$i)), 1.15, .835+(.72*(-$i)), 2.13);	# contact to node left
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), 1.15, .835+(.72*$i), 2.13);		# contact to node right
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115+(.72*(-$i+1)), 1.9, 1.555+(.72*(-$i+1)), 2.13);	# node interconnect left
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), 1.9, .115+(.72*($i+1)), 2.13);	# node interconnect right
		}
		$i++;
	}
	...
	until ($i >= ($max_folds + 1+$adjust+$pfolds + 2))
	{
		...
		{		
		      ...
		      if ($pfolds != 0)
		      {
		              ...
		              printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), .565, .835+(.72*$i), 1.15);	# contact to node right
              }
            if ($i == $max_folds + 1+$adjust)
		    {
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605-(.72*$i), 1.15, .835-(.72*$i), 2.13);	# contact to node left up
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), 1.15, .835+(.72*$i), 2.13);	# contact to node right up
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115-(.72*($i-1)), 1.9, 1.555-(.72*($i-1)), 2.13);	# node interconnect left
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), 1.9, .115+(.72*($i+1)), 2.13);	# node interconnect right
		    }
		    else
		    {
			     ...
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), .565, .115+(.72*($i+1)), .795);	# node interconnect right
		    }
		}
		$i++;
	...
	until ($i >= ($max_folds + $adjust + 2))
	{
		...
		printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), -1.63, .115+(.72*($i+1)), -1.4);	# node interconnect right
		$i++;
		if ($i < ($max_folds + 2))
		{
...
######## METAL2
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.3+.72+.72*(2*$max_folds+$adjust), -.06, 1.585+.72+.72*(2*$max_folds+$adjust), .32);	# out metal
...
######## VIA
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.315+.72+.72*(2*$max_folds+$adjust), 0, 1.575+.72+.72*(2*$max_folds+$adjust), .26);		# out via						# in0 pin
