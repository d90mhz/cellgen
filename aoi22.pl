...
sub aoi22	# parameters: lef filehandle, cell name, lambda, height, ratio, psize, nsize
{
...
######## METAL1
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.325+.72+.72*(2*$max_folds+$adjust), 0, 1.555+.72+.72*(2*$max_folds+$adjust), .795);	# out to p-out
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.325, .565, 1.555+.72+.72*(2*$max_folds+$adjust), .795);	# out interconnect right
	if ($nfolds == 0)
	{
	       printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115+.72*($nfolds+3), -.56, .115+.72*($nfolds+3), .795);	# p-out to n-out connect wire
    }
    else
    {
	       printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115+.72*($nfolds+1), -.56, .115+.72*($nfolds+1), .795);	# p-out to n-out connect wire
    }
	## m1 for nmos side ######
	####### for in0 and in1
	$i = 0;
	until ($i >= ($max_folds + $adjust +2))
	{
		...
		{
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115-(.72*($i-1)), -1.63, 1.555-(.72*($i-1)), -1.4);	# node left interconnect
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), -1.63, .115+(.72*($i+1)), -1.4);	# node right interconnect
		}
		$i++;
	}
	$i = 0;
	...
	until ($i >= ($max_folds + 1+$adjust+$nfolds + 2))
	{
		...
		{		
		      ...
		      if ($nfolds != 0)
		      {
		              ...
		              printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), -.905, .835+(.72*$i), -.33);	# contact to node right
              }
            if ($i == $max_folds + 1+$adjust)
		    {
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605-(.72*$i), -1.63, .835-(.72*$i), -.905);	# contact to node left down
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), -1.63, .835+(.72*$i), -.905);	# contact to node right down
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115-(.72*($i-1)), -1.63, 1.555-(.72*($i-1)), -1.4);	# node interconnect left
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), -1.63, .115+(.72*($i+1)), -1.4);	# node interconnect right
		    }
		    else
		    {
			     ...
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), -.56, .115+(.72*($i+1)), -.33);	# node interconnect right
		    }
		}
		$i++;
