...
sub nor3	# parameters: lef filehandle, cell name, lambda, height, ratio, psize, nsize
{
	...
	my $adjust = (isodd($pfolds) && ($pfolds >= $nfolds));
	
	print $lefhandle "MACRO $cell_name\n  CLASS CORE ;\n";
	@lcoords = lefcoords ($lambda, -.36-.72*$max_folds, -2.27, 1.8+.72*(2*$max_folds+$adjust+1), 2.77);			# cell boundary
	...
	@lcoords = lefcoords ($lambda, -.36-.72*$max_folds, 2.37, 1.8+.72+.72*(2*$max_folds+$adjust), 3.17);		# pin vdd!
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", $lcoords[0], $lcoords[1], $lcoords[2], $lcoords[3];
	print $lefhandle "    END\n  END vdd!\n  PIN gnd!\n    DIRECTION INOUT ;\n    USE GROUND ;\n    SHAPE ABUTMENT ;\n ";
	print $lefhandle "   PORT\n    LAYER METAL1 ;\n";
	@lcoords = lefcoords ($lambda, -.36-.72*$max_folds, -2.67, 1.8+.72+.72*(2*$max_folds+$adjust), -1.87);		# pin gnd!
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", $lcoords[0], $lcoords[1], $lcoords[2], $lcoords[3];
	print $lefhandle "    END\n  END gnd!\n  PIN out\n    DIRECTION OUTPUT ;\n    PORT\n    LAYER METAL2 ;\n";
...
######## NWELL
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.79-.29-.72*$max_folds, 0, 2.23+.29+.72+.72*(2*$max_folds+$adjust), 3.31);		# NWELL

######## NIMP
	print $lefhandle "    LAYER NIMP ;\n";
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.39-.72*$nfolds, -1.725, 1.83+.72+.72*($max_folds+$nfolds+$adjust), 0);
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.46-.72*$max_folds, 2.225, 1.9+.72+.72*(2*$max_folds+$adjust), 2.99);

######## PIMP
	print $lefhandle "    LAYER PIMP ;\n";
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.39-.72*$pfolds, 0, 1.83+.72+.72*($max_folds+$pfolds+$adjust), 2.225);
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.46-.72*$max_folds, -2.49, 1.9+.72+.72*(2*$max_folds+$adjust), -1.725);
...
######## DIFF
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.36-.72*$max_folds, 2.325, 1.8+.72+.72*(2*$max_folds+$adjust), 2.88);	# NDIFF for vdd
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$nfolds, -.905-(floor(100*$nsize_each)/200), 1.65+.72*($max_folds+$adjust+$nfolds+1), -.905+(floor((100*$nsize_each)+.5)/200));	# NDIFF for nmos

	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.36-.72*$max_folds, -2.38, 1.8+.72+.72*(2*$max_folds+$adjust), -1.825);	# PDIFF for gnd
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21+.72+.72*($max_folds+1+$adjust), 1.15-(floor(100*$psize_each)/200), 1.65+.72+.72*($max_folds+$pfolds+$adjust), 1.15+(floor((100*$psize_each)+.5)/200));	# PDIFF for pmos right
	if (isodd($pfolds))
	{
		printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$pfolds, 1.15-(floor(100*$psize_each)/200), 1.65+.72*$pfolds, 1.15+(floor((100*$psize_each)+.5)/200));	# PDIFF for pmos left
	}
	else
	{
		printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$pfolds, 1.15-(floor(100*$psize_each)/200), 1.65+.72*$max_folds, 1.15+(floor((100*$psize_each)+.5)/200));	# PDIFF for pmos left
	}
...
######## METAL1
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.36-.72*$max_folds, 2.37, 1.8+.72+.72*(2*$max_folds+$adjust), 3.17);		# vdd metal
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.36-.72*$max_folds, -2.67, 1.8+.72+.72*(2*$max_folds+$adjust), -1.87);		# gnd metal
	...
	until ($i >= ($max_folds + $adjust + 2))
	{
		...
		{
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), .565, .115+(.72*($i+1)), .795);	# node2 interconnect right
		}
		$i++;
	}
	$i = 0;
	until ($i >= ($pfolds + 2))
	{
		if (($i == 0 && $pfolds != 0) || ($i != 0 && $max_folds != 0))
		{
			...
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*(-$i)), .565, .835+(.72*(-$i)), 1.15);	# contact to node1 left
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), 1.15, .835+(.72*$i), 2.13);		# contact to node1 right
		}
		if ($i != 0)
		{
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115+(.72*(-$i+1)), .565, 1.555+.72+(.72*(-$i)), .795);	# node1 interconnect left
		...
		if ($i < ($pfolds + 2))
		{
			...
			{
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .55+(.72*$i), 1.035, .89+(.72*$i), 1.265);	# contact cover right
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), .565, .835+(.72*$i), 1.15);		# contact to node2 right
				if ($i != 1)
					{
						printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), .565, .115+(.72*($i+1)), .795);	# node2 interconnect right
					}
	...
	until ($i >= ($max_folds + 1+$adjust+$pfolds + 2))
	{
		if ($max_folds != 0)
		{
			...
			if ($pfolds != 0)
			{
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), 1.15, .835+(.72*$i), 2.13);	# contact to node right
			}
			if ($i != $max_folds + 1+$adjust)
			{
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), 1.9, .115+(.72*($i+1)), 2.13);	# node2 interconnect right
			}
			if ($i == $max_folds + 1+$adjust)
			{
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.835+(.72*$i), .565, .835+(.72*$i), .795);	# node2 interconnect right
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), .565, .835+(.72*$i), 1.15);	# contact to node down right
			}
		}
		$i++;
	...
	until ($i >= ($max_folds + $adjust + 2))
	{
		...
		if ($i < ($max_folds + 2))
		{
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), -.56, .115+(.72*($i+1)), -.33);	# out interconnect right
		}
		$i++;
	}
	$i = 0;
	until ($i >= ($nfolds + 2))
	{
		...
		if ($i < ($nfolds + 2))
		{
			...
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115+.72+(.72*(-$i)), -.56, 1.555+.72+(.72*(-$i)), -.33);# out interconnect left
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), -.56, .115+(.72*($i+1)), -.33);	# out interconnect right
		}
		$i++;
	}
	...
	until ($i >= ($max_folds + 1+$adjust+$nfolds + 2))
	{
		if (isodd($nfolds))
		{
			...
			if ($i < ($max_folds + 1+$adjust+$nfolds + 2))
			{
				...
				{
					printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i)), -.56, .115+(.72*($i+1)), -.33);# out interconnect right
				}
			}
			$i++;
		}
		else
		{
			...
			{
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*$i), -.905, .835+(.72*$i), -.33);		# contact to out right
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -1.555+(.72*($i+1)), -.56, .115+(.72*($i+2)), -.33);	# out interconnect right
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*($i+1)), -.56, .835+(.72*($i+1)), 0);	# interconnect to out right
			}
			$i++;
			if ($i < ($max_folds + 1+$adjust+$nfolds + 2))
			{
