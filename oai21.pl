...
sub oai21	# parameters: lef filehandle, cell name, lambda, height, ratio, psize, nsize
{
...
######## DIFF
	...
	if (iseven($pfolds))
	{	
		printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$max_folds, 1.15-(floor(100*$psize_each)/200), 1.65+.72*$pfolds, 1.15+(floor((100*$psize_each)+.5)/200));	# PDIFF for pmos middle if connected
	}
	else
	{
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72-.72*($max_folds+$pfolds+$adjust), 1.15-(floor(100*$psize_each)/200), 1.65-.72-.72*($max_folds+1+$adjust), 1.15+(floor((100*$psize_each)+.5)/200));	# PDIFF for nmos left

	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.36-.72*(2*$max_folds+$adjust+1), 2.325, 1.8+.72*($max_folds), 2.88);	# NDIFF for vdd
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*($max_folds+$adjust+$nfolds+1), -.905-(floor(100*$nsize_each)/200), 1.65+.72*($nfolds), -.905+(floor((100*$nsize_each)+.5)/200));	# NDIFF for pmos
...
######## METAL1
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.325, 0, 1.555, .795);	# out to p-out
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, 1.325-.72, .565, 1.555, .795);	# out to p-out
	...
	until ($i >= ($max_folds + 1+$adjust+$pfolds + 2))
	{
		...
		{		
            ...
            if ($i == $max_folds + 1+$adjust)
		    {
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605-(.72*$i), 1.15, .835-(.72*$i), 2.13);	# contact to node left down
			     printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.115-(.72*($i-1)), 1.9, 1.555-(.72*($i-1)),2.13);	# node interconnect left
		    }
		    else
		    {
	...
	until ($i >= ($max_folds + 1+$adjust+$nfolds + 2))
	{
		if (isodd($nfolds))
		{
			...
			{
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .55-(.72*$i), -1.02, .89-(.72*$i), -.79);	# contact cover left
				printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605-(.72*$i), -2.27, .835-(.72*$i), -.905);	# contact to vdd left
			}
			$i++;
		}
		else
