...
sub nand3	# parameters: lef filehandle, cell name, lambda, height, ratio, psize, nsize
{
...
######## DIFF
	...
	if (iseven($nfolds))
	{	
		printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$nfolds, -.905-(floor(100*$nsize_each)/200), 1.65+.72*$max_folds, -.905+(floor((100*$nsize_each)+.5)/200));	# NDIFF for nmos left if connected
	}
	else
	{
		printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$nfolds, -.905-(floor(100*$nsize_each)/200), 1.65+.72*$nfolds, -.905+(floor((100*$nsize_each)+.5)/200));	# NDIFF for nmos left if not connected
	}
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.21-.72*$pfolds, 1.15-(floor(100*$psize_each)/200), 1.65+.72*($max_folds+$adjust+$pfolds+1), 1.15+(floor((100*$psize_each)+.5)/200));	# PDIFF for pmos
