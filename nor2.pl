...
######## METAL1
	...
	until ($i >= ($pfolds + 2))
	{
		...
		if ($i != 0)
		{
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .605+(.72*(-$i)), .565, 2.275+(.72*(-$i)), .795);	# node interconnect left
			printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, -.835+(.72*$i), 1.9, .835+(.72*$i), 2.13);	# node interconnect right
		}
		$i++;
		if ($i < ($pfolds + 2))
		{
...
######## VIA
	...
	printf $lefhandle "    RECT %.3f %.3f %.3f %.3f ;\n", lefcoords ($lambda, .615, 0, .875, .26);		# in0 via						# out pin
